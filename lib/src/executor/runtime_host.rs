        ...
        match (self.inner.vm, self.inner.root_calculation.take()) {
            ...
            (host::HostVm::ExternalStorageAppend(req), None) => {
                let trie = match req.child_trie() {
                    None => &mut self.inner.pending_storage_changes.main_trie,
    ...
    pub fn inject_key(mut self, key: Option<impl Iterator<Item = Nibble>>) -> RuntimeHostVm {
        match (self.inner.vm, self.inner.root_calculation.take()) {
            (host::HostVm::ExternalStorageNextKey(req), None) => {
                ...
                let trie = match req.child_trie() {
                    None => Some(&self.inner.pending_storage_changes.main_trie),
                    Some(child_trie) => self
                        .inner
                        .pending_storage_changes
                        .default_child_tries
    ...
    fn run(mut self) -> RuntimeHostVm {
        loop {
                ...
                Some((trie, trie_root_calculator::InProgress::Finished { trie_root_hash })) => {
                    ...
                    if let Some(child_trie) = &trie {
                        ...
                        if trie_root_hash != trie::EMPTY_TRIE_MERKLE_VALUE {
                            self.pending_storage_changes.main_trie.diff_insert(
                                main_trie_key,
            ...
            // output to be accurate.
            {
                ...
                if let Some(child_trie_to_flush) = child_trie_to_flush {
                    // TODO: don't clone?
                    let diff = match self
                        .pending_storage_changes
                        .default_child_tries
                        .get(child_trie_to_flush)
                    {
                    ...
                    self.root_calculation = Some((
                        Some(child_trie_to_flush.clone()),
            ...
            match self.vm {
                ...
                host::HostVm::ExternalStorageSet(req) => {
                    ...
                    let trie = match req.child_trie() {
                        None => &mut self.pending_storage_changes.main_trie,
                        Some(child_trie) => self
                            .pending_storage_changes
                            .default_child_tries
                            .entry(child_trie.as_ref().to_vec())
