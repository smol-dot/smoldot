...
export async function startInstanceClient<A>(portToServer: MessagePort, eventCallback: (event: instance.Event<A>) => void): Promise<instance.Instance> {
    ...
    portToServer.onmessage = (messageEvent) => {
        const message = messageEvent.data as ServerToClient<A>;
...
export async function startInstanceServer<A>(config: instance.Config<A>, portToClient: MessagePort) {
    ...
    const eventsCallback = (event: instance.Event<A>) => {
        switch (event.ty) {
            case "add-chain-result": {
                if (event.success) {
        ...
        const ev: ServerToClient<A> = event;
    ...
    portToClient.onmessage = (messageEvent) => {
        ...
        switch (message.ty) {
            ...
            case "accept-more-json-rpc-answers": {
                ...
                if (response) {
                    const msg: ServerToClient<A> = { ty: "json-rpc-response", chainId: message.chainId, response };
                    portToClient.postMessage(msg);
                } else {
