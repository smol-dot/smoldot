...
export default (config: Config): WebAssembly.ModuleImports => {
    ...
    return {
        ...
        random_get: (ptr: number, len: number) => {
            ...
            const baseBuffer = new Uint8Array(config.memory.buffer).subarray(ptr, ptr + len);
            for (let iter = 0; iter < len; iter += 65536) {
                // `Uint8Array.subarray` automatically saturates at the end of the buffer
        ...
        fd_write: (fd: number, addr: number, num: number, outPtr: number) => {
            ...
            buffer.writeUInt32LE(new Uint8Array(config.memory.buffer), outPtr, totalLength);
            return 0;
        },
